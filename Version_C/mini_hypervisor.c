...
void *thread_body(void *data_ptr)
{
    ...
    char data;
    uint8_t operation = NO_OP;

    char *local_files_names[MAX_FILES_CNT];
    FILE *local_files[MAX_FILES_CNT];
    struct fileList lf;
    lf.cnt = 0;
    lf.list = local_files;
    lf.names = local_files_names;
    struct fileList *localFiles = &lf;
    int fileHandlersRetVal;

    struct pathName pn;
    struct fwriteArgs fwa;
    struct fcloseArgs fca;
    struct freadArgs fra;
    ...
    while (stop == 0)
    {
        ...
        {
        case KVM_EXIT_IO:
            switch (vm.kvm_run->io.port)
            {
            case 0x0278:
                if (vm.kvm_run->io.direction == KVM_EXIT_IO_OUT)
                {
                    if (operation == NO_OP)
                        operation = *((uint8_t *)vm.kvm_run + vm.kvm_run->io.data_offset);
                    else if (operation == FOPEN_CODE)
                        handle_fopen(&fileHandlersRetVal, &vm, localFiles, globFiles, &pn);
                    else if (operation == FWRITE_CODE)
                        handle_fwrite(&fileHandlersRetVal, &vm, localFiles, globFiles, &fwa, charId);
                    else if (operation == FCLOSE_CODE)
                        handle_fclose(&fileHandlersRetVal, &vm, localFiles, globFiles, &fca);
                    else if (operation == FREAD_CODE)
                        handle_fread(&fileHandlersRetVal, &vm, localFiles, globFiles, &fra);
                }
                else if (vm.kvm_run->io.direction == KVM_EXIT_IO_IN)
                {
                    if (operation == FOPEN_CODE)
                    {
                        char *data_in = (((char *)vm.kvm_run) + vm.kvm_run->io.data_offset);
                        (*data_in) = fileHandlersRetVal;
                        operation = NO_OP;
                    }
                    else if (operation == FWRITE_CODE || operation == FCLOSE_CODE)
                    {
                        char *data_in = (((char *)vm.kvm_run) + vm.kvm_run->io.data_offset);
                        (*data_in) = fileHandlersRetVal;
                        operation = NO_OP;
                    ...
                    {
                        char *data_in = (((char *)vm.kvm_run) + vm.kvm_run->io.data_offset);
                        (*data_in) = fra.readBuffer[fra.cntRead];

                        fra.cntRead++;
                        if (fra.cntRead == fra.cntToRead)
                        {
                            fra.cntToRead = 0;
                            free(fra.readBuffer);
                            operation = NO_OP;
            ...
            case 0xE9:
                ...
                if (vm.kvm_run->io.direction == KVM_EXIT_IO_OUT)
                {
                    printf("%d", *((char *)vm.kvm_run + vm.kvm_run->io.data_offset));
                }
                else if (vm.kvm_run->io.direction == KVM_EXIT_IO_IN)
                {
                    scanf("%c", &data);
    ...
    {
        free(localFiles->names[i]);
    }
}

int main(int argc, char *argv[])
{
    struct thread_data thread_data[MAX_FILES_CNT];
    int num_of_threads = 0;

    struct fileList globFiles;

    char *global_files_names[MAX_FILES_CNT];
    FILE *global_files[MAX_FILES_CNT];
    int num_of_glob_files = 0;
    ...
    for (int i = 1; i < argc;)
    {
        ...
        else if (strcmp(argv[i], "--file") == 0 || strcmp(argv[i], "-f") == 0)
        {
            while (++i < argc)
            {
                if (argv[i][0] == '-')
                    break;
                global_files_names[num_of_glob_files++] = argv[i];
            }
        }
        else
        {
    ...
    for (int i = 0; i < num_of_glob_files; i++)
    {
        global_files[i] = fopen(global_files_names[i], "r");
        if (global_files[i] == NULL)
        {
            printf("Can not open global file %s\n", global_files_names[i]);
            exit(-1);
        }
    }

    globFiles.cnt = num_of_glob_files;
    globFiles.list = global_files;
    globFiles.names = global_files_names;
    ...
    for (int i = 0; i < num_of_threads; i++)
    {
        ...
        thread_data[i].globFiles = &globFiles;
