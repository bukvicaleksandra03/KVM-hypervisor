...
void usage() {
    printf("Usage: ./mini_hypervisor --memory <2|4|8> --page <4KB|2MB> --guest <guest.img>\n");
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if (img_name == NULL) {
        printf("Guest image not specified.\n");
        usage();
    }
		...
		switch (vm.kvm_run->exit_reason) {
			...
			case KVM_EXIT_IO:
				if (vm.kvm_run->io.direction == KVM_EXIT_IO_OUT && vm.kvm_run->io.port == 0xE9) {
                    ...
                    //printf("Count: %d Size: %d Offset: %lld \n", vm.kvm_run->io.count, vm.kvm_run->io.size, vm.kvm_run->io.data_offset); 
                    scanf("%c", &data);
